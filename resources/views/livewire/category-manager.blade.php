    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
            ...
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Gestión de Categorías</h2>
                <div class="flex gap-2">
                    {{-- Export Buttons --}}
                    <a href="{{ route('export.categories.csv', ['search' => $search, 'status' => $filterStatus]) }}"
                        class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm">
                        CSV
                    </a>
                    <a href="{{ route('export.categories.excel', ['search' => $search, 'status' => $filterStatus]) }}"
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-gray-100">
                                    <span class="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                                        {{ $item->products_count }} producto{{ $item->products_count != 1 ? 's' : '' }}
                                    </span>
                                </td>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-gray-600/50 overflow-y-auto h-full w-full z-50" wire:click="$set('showModal', false)">
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800 dark:border-gray-700" wire:click.stop>
                <h3 class="text-lg font-medium mb-4 text-gray-900 dark:text-gray-100">{{ $isEditing ? 'Editar' : 'Crear' }} Categoría</h3>
        ...
        <div class="fixed inset-0 bg-gray-600/50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
            <div class="mx-auto p-6 border w-96 shadow-lg rounded-lg bg-white dark:bg-gray-800 dark:border-gray-700" wire:click.stop>
                ...
                <p class="text-sm text-center text-gray-600 dark:text-gray-400 mb-6">
                    ¿Estás seguro de que querés eliminar <strong class="text-gray-900 dark:text-gray-100">{{ $deleteCategoryName }}</strong>? Esta acción no se puede deshacer.
                </p>
